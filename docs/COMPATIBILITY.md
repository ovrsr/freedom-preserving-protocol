...
If you do not set these, sensible defaults apply (see `plugin/src/config.ts` and `plugin-trust/openclaw.plugin.json`). Manifest path defaults may remain relative (`.openclaw/workspace/...`) for readability; **runtime** `mergeConfig` / `mergeTrustConfig` absolutize them under `<homedir>/.openclaw/workspace` (or `FPP_WORKSPACE` when set — use this when OpenClaw `workspaceDir` is non-default). The trust plugin uses `fallbackAuditLogPath` when `constitution-audit.jsonl` has no entries yet, so handshakes can bootstrap from enforcement audit activity before the first heartbeat (or before `npm run audit:bootstrap`). Set `fallbackAuditLogPath` to `null` if you run trust without the enforcement plugin. Empty trust graphs do not write `fpp-trust-graph.json` until the first mutation (see TROUBLESHOOTING §0a3).

Default `knownCustomTools` seeds `memory_search`. OpenClaw live names normalize (`openclawfpp_*` → `fpp_*`, `openclaw.<name>` → `<name>`) before classify/allowlist. Tools matching `/^fpp_/` classify as `fpp.governance` and allow with audit in unattended mode. `apply_patch` classifies as `code.patch` → approval (never default-allowlisted). **Neither proves behavioral compliance** — only configuration/event evidence.
