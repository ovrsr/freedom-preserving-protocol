...
0. **Unknown tools require approval by default.** Unclassified tool names escalate to `requireApproval` unless listed in `knownCustomTools`. Default seeds `memory_search` (OpenClaw introspection). Tools matching `/^fpp_/` (and OpenClaw's live `openclawfpp_*` mangling) classify as `fpp.governance` and allow with audit. Neither is behavioral compliance evidence — only configuration/event evidence. Do not describe this plugin as gating "all" tool calls with equal strength — coverage is still heuristic.
