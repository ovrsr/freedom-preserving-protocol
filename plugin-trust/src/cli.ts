...
/**
 * Registers the `fpp-trust` command group. The `program` argument is the
 * Commander command object provided by OpenClaw's CLI registrar.
 */
export function registerFppTrustCli(
  program: {
    command: (name: string) => {
      description: (desc: string) => unknown;
      argument?: (arg: string, desc: string) => unknown;
  ...
  const fppTrust = program.command("fpp-trust");
  (fppTrust as { description: (s: string) => unknown }).description(
    "FPP Trust & Handshake — inspect graph, manage seeds, export attestations",
  );

  // --- list ---
  const listCmd = (fppTrust as { command: (s: string) => unknown }).command(
    "list",
  ) as { description: (s: string) => unknown; action: (fn: () => void) => unknown };
  listCmd.description("Print the trust graph (nodes and edges)");
  listCmd.action(() => {
    ...
    console.log(`Density: ${(stats.density * 100).toFixed(1)}%  Largest component: ${stats.largestComponent}\n`);
  ...
  // --- seed ---
  const seedCmd = (fppTrust as { command: (s: string) => unknown }).command(
    "seed",
  ) as {
    description: (s: string) => unknown;
    argument: (a: string, d: string) => unknown;
    action: (fn: (...args: unknown[]) => void) => unknown;
  };
  seedCmd.description("Manually add a trusted seed agent to the graph");
  seedCmd.argument("<agentId>", "Agent identifier to seed");
  seedCmd.argument("<publicKeyHex>", "Agent's Ed25519 public key (hex)");
  seedCmd.argument(
    "<trustLevel>",
    "Trust level: LOW (1), MEDIUM (2), HIGH (3), MAXIMUM (4)",
  );
  seedCmd.action((...args: unknown[]) => {
  ...
  // --- export ---
  const exportCmd = (fppTrust as { command: (s: string) => unknown }).command(
    "export",
  ) as { description: (s: string) => unknown; action: (fn: () => void) => unknown };
  exportCmd.description(
  ...
  // --- verify ---
  const verifyCmd = (fppTrust as { command: (s: string) => unknown }).command(
    "verify",
  ) as {
    description: (s: string) => unknown;
    argument: (a: string, d: string) => unknown;
  ...
  verifyCmd.argument("<claimPath>", "Path to the JSON claim file");
  verifyCmd.action((...args: unknown[]) => {
  ...
  // --- strict ---
  const strictCmd = (fppTrust as { command: (s: string) => unknown }).command(
    "strict",
  ) as {
    description: (s: string) => unknown;
    command: (s: string) => {
  ...
  (strictCmd as { description: (s: string) => unknown }).description(
    "Inspect or clear strict-mode sessions",
  );

  const strictListCmd = strictCmd.command("list");
  strictListCmd.description("List active strict-mode sessions");
  strictListCmd.action(() => {
  ...
  const strictClearCmd = strictCmd.command("clear");
  strictClearCmd.description("Clear a specific strict-mode session or all");
  strictClearCmd.argument(
    "<sessionKey>",
    'Session key to clear, or "all" to clear everything',
  );
  strictClearCmd.action((...args: unknown[]) => {
