 ...
 *   6. The companion plugin's presence is checked by invoking
 *      `openclaw plugins list --json` if `openclaw` is on PATH. Absence is
 *      reported as "dispatcher layer not active" — not an error.
 *
...
function checkPluginInstalled(): Check {
  ...
  const present = PLUGIN_ID_CANDIDATES.some((id) => stdout.includes(id));
  return {
    id: "plugin.installed",
    label: "Dispatcher-layer plugin",
    ...
    detail: present
      ...
      : "FPP plugin NOT installed — only prompt-layer governance is active. Install with: openclaw plugins install clawhub:ovrsr/openclaw-fpp-plugin",
  };
}
...
function main() {
  ...
  checks.push(checkPluginInstalled());
  ...
  const dispatcherLayerActive =
    checks.find((c) => c.id === "plugin.installed")?.status === "pass";

  const report: Report = {
    ok: requiredOk,
    checks,
    summary: { promptLayerActive, dispatcherLayerActive },
  };
